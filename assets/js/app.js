...
$(function(){
    ...
    SingAppView.prototype._loadScripts = function(event, data, status, xhr, options){
        var $headContents = $($.parseHTML(data.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0], document, true)),
            $bodyContents = $($.parseHTML(data.match(/<body[^>]*>([\s\S.]*)<\/body>/i)[0], document, true)),
            $stylesheets = $headContents.filter('link[rel="stylesheet"]').add($headContents.find('link[rel="stylesheet"]')),
